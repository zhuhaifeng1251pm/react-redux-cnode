    ...
    render() {
        ...
        const isReadMessage = isRead.length ? (
            <div className="isReadMessage ">
                ...
                <ul>
                    {isRead.length
                        ? isRead.map(t => <li key={t.id}><NavLink to={`/user/${t.author.loginname}`}>{t.author.loginname}</NavLink>回复了你的话题<NavLink to={`/topic/${t.topic.id}`} className='title'>{t.topic.title}</NavLink><div style={{display:t.type==='at'?'block':'none'}}><span >@</span><span >{name}</span></div></li>)
                        : <span>无消息</span>}
                </ul>
            </div>
        ) : (
        ...
        return (
            <Wrap>
                <div className="left">{isNoreadMessage}{isReadMessage}</div>
                <div className="right">
                    <p
                        style={{
                            color: "#51585c",
